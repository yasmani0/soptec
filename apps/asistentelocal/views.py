...
def index(request):
    is_admin_asistente_local = User.objects.filter(
        id=request.user.id)
    is_tipo_usuario = Cliente.objects.filter(
        id_cliente=request.user.id, tipo_usuario=3)
    if is_admin_asistente_local and is_tipo_usuario:
        ...
        usuariosReg = Cliente.objects.filter(tipo_usuario=1).count()
        ...
        return render(request, 'base_local_asistente/base.html', contexto)
    else:
        messages.error(request, "Acceso denegado")
        return redirect('usuario_salir')


def update_cuenta_local_asistente(request):
...
def local_asistente_admuser(request):
    if request.user.is_authenticated:
        ...
        if is_admin and is_tipo_usuario:
            ...
            usuarios = User.objects.raw(
                "select c.tipo_usuario, u.id, u.username, u.first_name, u.last_name, u.email, u.is_active from cliente_cliente as c inner join  auth_user as u on c.id_cliente_id=u.id where c.tipo_usuario='1' ORDER BY id desc")
            user = {'usuarios': usuarios, 'datos_cuenta': datos_cuenta,
                    'form_cuenta': form_cuenta}

            return render(request, 'LocalAsistentesFuncionalidades/usuario/index.html', user)
        else:
            messages.error(request, "Acceso denegado")
            return redirect('usuario_salir')


def local_asistente_usuario_register_adm(request):
    is_admin_local = User.objects.filter(
        id=request.user.id)
    is_tipo_usuario = Cliente.objects.filter(
        id_cliente=request.user.id, tipo_usuario=3)
...
def local_asistente_usuario_actualizar_adm(request):
    is_admin_local = User.objects.filter(
        id=request.user.id)
    is_tipo_usuario = Cliente.objects.filter(
        id_cliente=request.user.id, tipo_usuario=3)
    if is_admin_local and is_tipo_usuario:
        ...
        messages.error(request, "Acceso denegados")
        return redirect('usuario_salir')
